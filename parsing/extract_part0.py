...
def interpret(contents, DATA, bbo_file):
    ...
    while i < len(contents):
        ...
        if eap.is_command(contents[i]):
            ...
            elif eap.commands[contents[i]] == 5:  # qx, start of hands (some files do not have the command md, the
                # start of hands, and we will treat them here)
    ...
    for i in range(len(hands_texts)):
        ...
        if (hands != eap.error['hands']) and (dealer != eap.error['dealer']):  # error check
            ...
            if (leader != eap.error['leader']) and (biddings_encoded != eap.error['bidding']):  # error check
                ...
                DATA.append(BridgeDeal(bbo_file, None, None, game[i], biddings_encoded, leader, dealer, leads[i],
                                       biddings[i][0]))
